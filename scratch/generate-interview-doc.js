...
const targetPath = path.join(__dirname, "../INTERVIEW.txt");
...
$$\\boxed{T = \\text{Actual Genuine PDF / Book Reading Time}}$$

$$\\begin{aligned}
\\text{Reading Time} &\\equiv T \\\\
\\text{Diya / Streak Time} &\\equiv T \\\\
\\text{Total Active Time} &= T + \\text{Website Exploration Time} \\\\
\\text{Active Time} &\\ge \\text{Reading Time} \\quad (\\forall \\text{ days and all-time})
\\end{aligned}$$
...
- 15:00+ minutes of daily reading ($T \\ge 900\\text{s}$): Diya illuminates with an authentic, 
...
--------------------------------------
1. In-Memory Write-Through Caching: Every read first hits high-speed in-memory JavaScript \`Map\`s. 
...
1. Direct DOM Pointer Transforms on Mouse Movement:
   ...
   - Solution: In \`CustomCursor.tsx\` and \`CardTilt.tsx\`, mouse coordinates update component \`useRef\`s 
...
$$x_{\\text{normalized}} = \\frac{x_{\\text{canvas}}}{\\text{pageWidth}}, \\quad y_{\\text{normalized}} = \\frac{y_{\\text{canvas}}}{\\text{pageHeight}}$$
...
$x_{\\text{normalized}} \\times \\text{newWidth}$ karke exact usi word pe annotation render karte hain."
...
3. In-Memory Cache Pattern (Write-Through Caching): Caches localStorage data in \`Map\`s for 0ms reads.
...
4. User completes 15 minutes of reading:
   \`BookReader\` timer ticks -> $T \\ge 900\\text{s}$ -> \`isTodayQualified: true\` -> \`DiwaliDiya.tsx\` renders living flame.
...
- Result: $\\text{Active Time} \\ge \\text{Reading Time}$ invariant holds 100% across all scenarios.
...
Detailed Answer:
\`distToPolyline\` iterates through all line segments, calculating the orthogonal projection point $t = \\frac{(P - A) \\cdot (B - A)}{|B - A|^2}$ clamped between 0 and 1. If the shortest distance to the cursor is within the selection threshold (10px), the stroke is selected.
Hinglish Intuition:
...
Detailed Answer:
When a shape is selected, its bounding box (\`minX\`, \`minY\`, \`maxX\`, \`maxY\`) is calculated. Eight handle points are rendered. Dragging a handle (e.g., \`se\` for south-east) computes scale deltas ($\\Delta x, \\Delta y$) and transforms all constituent points relative to the opposite fixed anchor.
Hinglish Intuition:
...
$$\\text{Reading Time} \\equiv \\text{Diya/Streak Time} \\equiv T$$
$$\\text{Active Time} = T + \\text{Exploration Time} \\ge \\text{Reading Time}$$
...
Short Answer:
It checks if the user has accumulated $\\ge 15$ minutes (900 seconds) of genuine reading time ($T$) on the current calendar day.
Detailed Answer:
Each day's reading seconds are stored in \`readershub:reading-activity:v1\` under the key \`YYYY-MM-DD\`. Once $T \\ge 900\\text{s}$, \`qualified\` becomes \`true\` and the Diya flame renders with animated CSS sway keyframes. The current streak counts consecutive qualified days backward from today/yesterday.
Hinglish Intuition:
...
| **What is the Timing Invariant?** | $\\text{Reading Time} \\equiv \\text{Diya Time} = T$, while $\\text{Active Time} = T + \\text{Exploration Time}$. Active Time is mathematically guaranteed to be $\\ge \\text{Reading Time}$. |
| **How does Diya Streak work?** | Accumulating $\\ge 15$ minutes (900s) of genuine PDF reading on a calendar day lights the living Diya flame and increments the consecutive daily streak. |
...
Trap 3: "How do you prove that your timing invariant $\\text{Active Time} \\ge \\text{Reading Time}$ actually holds?"
Defensive Strategy:
...
console.log("✅ Successfully created INTERVIEW.txt at:", targetPath);
console.log("Total bytes written:", Buffer.byteLength(content, "utf8"));
